...
unsigned int Device_Parameter_Set::LFU_reset_interval = 100000000;
...
void Device_Parameter_Set::XML_serialize(Utils::XmlWriter& xmlwriter)
{
	...
	attr = "LFU_reset_interval";
	val = std::to_string(LFU_reset_interval);
	xmlwriter.Write_attribute_string(attr, val);
...
void Device_Parameter_Set::XML_deserialize(rapidxml::xml_node<> *node)
{
	try
	{
		for (auto param = node->first_node(); param; param = param->next_sibling()) {
			...
			} else if (strcmp(param->name(), "Data_Cache_Evict_Policy") == 0){
				std::string val = param->value();
				std::transform(val.begin(), val.end(), val.begin(), ::toupper);
				if(strcmp(val.c_str(), "LRU") == 0){
					LFU = false;
				} else if(strcmp(val.c_str(), "LFU") == 0){
					LFU = true;
				} 	else{
